...
const ClickStats = ({ apiBaseUrl, token, shortUrl, showAlert }) => {
  ...
  const fetchClickData = async () => {
    try {
      ...
      showAlert('Error fetching click data: ' + error.response?.data?.detail, 'error');
    }
  };
  ...
  const processClickData = () => {
    ...
    clickData.forEach((timestamp) => {
      ...
      switch (grouping) {
        case 'minute':
          key = dateObj.toUTCString().slice(0, 11) + dateObj.toUTCString().slice(16, 22);
          break;
        case 'hour':
          key = dateObj.toUTCString().slice(0, 11) + dateObj.toUTCString().slice(16, 19);
          break;
        case 'day':
        default:
  ...
  return (
    <div className="click-stats">
      <div className="grouping-controls">
        <button
          className={`grouping-btn ${grouping === 'day' ? 'active' : ''}`}
          onClick={() => setGrouping('day')}
        >
          By days
        </button>
        <button
          className={`grouping-btn ${grouping === 'hour' ? 'active' : ''}`}
          onClick={() => setGrouping('hour')}
        >
          By hours
        </button>
        <button
          className={`grouping-btn ${grouping === 'minute' ? 'active' : ''}`}
      ...
      {clickData.length === 0 ? (
        ...
        <Line
          data={{
            ...
            datasets: [
              {
                label: `Clicks per ${grouping === 'day' ? 'Day' : grouping === 'hour' ? 'Hour' : 'Minute'}`,
                data,
          ...
          options={{
            ...
            scales: {
              x: { title: { display: true, text: grouping === 'minute' ? 'Minute' : grouping === 'hour' ? 'Hour' : 'Date' } },
