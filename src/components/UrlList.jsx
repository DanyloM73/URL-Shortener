...
const UrlList = ({ apiBaseUrl, token, showAlert }) => {
  ...
  return (
    <div className='url-list'>
      ...
      {urls.length === 0 ? (
        <p className='no-urls-text'>Here you can see a list of all the URLs you have shortened.</p>
      ) : (
        <ul>
          {urls.map((url, index) => (
            <li className='list-item' key={index}>
              <div>
                <a href={`${apiBaseUrl}/${url.short}`} target="_blank" rel="noopener noreferrer">
                  {`${apiBaseUrl}/${url.short}`}
                </a>
                {selectedShortUrl === url.short && (
                  <ClickStats apiBaseUrl={apiBaseUrl} token={token} shortUrl={url.short} />
                )}
              </div>
              <button className='stats-btn' onClick={() => handleStatsToggle(url.short)}>
                {selectedShortUrl === url.short ? 'Hide Stats' : 'View Stats'}
              </button>
            </li>
          ))}
        </ul>
